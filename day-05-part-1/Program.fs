...
// Historians continue their search of this historically significant facility, an Elf operating a very
...
// manual updates won't print correctly. Failure to update the safety manuals would be dire indeed, so
// you offer your services.
//
// Safety protocols clearly indicate that new pages for the safety manuals must be printed in a very
// specific order. The notation X|Y means that if both page number X and page number Y are to be produced
...
// not include every page number, and so only some of the ordering rules apply - within each update, the ordering rules that
...
let orderingLines = File.ReadAllLines "./ordering-rules.txt"
let updatePagesLines = File.ReadAllLines "./update-pages.txt"
...
let orderedUpdates = updates |> List.filter isUpdateOrdered
let middleValues = orderedUpdates |> List.map (fun values -> values.Item(values.Length / 2))
