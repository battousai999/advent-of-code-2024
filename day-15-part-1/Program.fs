...
//
// Because lanternfish populations grow rapidly, they need a lot of food, and that food needs to be stored somewhere. That's
// why these lanternfish have built elaborate warehouse complexes operated by robots!
//
...
// if this action would cause the robot or a box to move into a wall (#), nothing moves instead, including the robot. The initial positions
// of these are shown on the map at the top of the document the lanternfish gave you.
//
// The rest of the document describes the moves (^ for up, v for down, < for left, > for right) that the robot will attempt to make,
// in order. (The moves form a single giant sequence; they are broken into multiple lines just to make copy-pasting easier. Newlines within
// the move sequence should be ignored.)
//
...
// of a box is equal to 100 times its distance from the top edge of the map plus its distance from the left edge of the map. (This process
// does not stop at wall tiles; measure all the way to the edges of the map.)
//
// So, the box shown below has a distance of 1 from the top edge of the map and 4 from the left edge of the map, resulting in a GPS coordinate
// of 100 * 1 + 4 = 104.
//
// #######
// #...O..
// #......
//
// The lanternfish would like to know the sum of all boxes' GPS coordinates after the robot finishes moving. In the larger example, the sum
// of all boxes' GPS coordinates is 10092. In the smaller example, the sum is 2028.
//
