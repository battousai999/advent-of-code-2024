...
let rawDiskMap = File.ReadAllText "./input.txt"
...
let compactDiskMap (diskMap: DiskBlock array) =
    ...
    while i < diskMap.Length - 1 do
        match diskMap.[i], diskMap.[i + 1] with
        | File _, FreeSpace ->
            let lastDiskBlockIndex = diskMap |> Array.findIndexBack (fun x -> x <> Empty && x <> FreeSpace)
            diskMap.[i + 1] <- diskMap.[lastDiskBlockIndex]
            diskMap.[lastDiskBlockIndex] <- Empty
        | _, Empty -> i <- Int32.MaxValue - 1
        | _ -> ()
